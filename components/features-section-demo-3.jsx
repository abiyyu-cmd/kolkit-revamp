    ...
    <div className="w-full p-5 mx-auto bg-white dark:bg-neutral-900 shadow-2xl group h-full">
      <div className="flex flex-1 w-full h-full flex-col space-y-2">
        <Image
          ...
          width={800}
          height={800}
          className="h-full w-full aspect-square object-top-left rounded-sm"
        />
      </div>
    </div>
...
export const SkeletonTwo = () => {
  ...
  // generate rotations only once using useState initializer (ESLint-safe)
  const [rotations] = useState(() =>
    Array.from({ length: images.length * 2 }, () => Math.random() * 20 - 10)
  );
      ...
      <div className="flex flex-row">
        {images.map((image, idx) => (
          <motion.div
            ...
            variants={imageVariants}
            style={{ rotate: rotations[idx + images.length] }}
            whileHover="whileHover"
            whileTap="whileTap"
            className="rounded-xl -mr-4 mt-4 p-1 bg-white dark:bg-neutral-800 dark:border-neutral-700 border border-neutral-100 shrink-0 overflow-hidden"
          >
